...
export default async function handler(req, res) {
  ...
  // Покупательское намерение проходит всегда, лимит для него не существует
  const buyingIntent = /куп|оплат|беру|возьму|заплат|плачу/i.test(question);
  if (used >= limit && !buyingIntent) {
